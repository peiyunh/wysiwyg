...
class NuScenesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # print(f"Calling NuScenes get_sensor_data({idx})")
        # print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # print(f"!!!!!!!!!!!! NEVER MIND I AM FIXING IT TO 3999 !!!!!!!!!!!!")
        # idx = 3999
        # print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    def get_sensor_data(self, query):
        ...
        for sweep in info["sweeps"]:
            ...
            sweep_origins_list.append(sweep["sweep2lidar_translation"])

        points = np.concatenate(sweep_points_list, axis=0)[:, [0, 1, 2, 4]]
        indices = np.cumsum([0] + [len(pts) for pts in sweep_points_list]).astype(int)
        ...
        res["lidar"]["indices"] = indices
        res["lidar"]["origins"] = origins
        if 'gt_boxes' in info:
            ...
            if "gt_attrs" in info:
                res["lidar"]["annotations"]["attrs"] = info["gt_attrs"][mask]

        if 'sweep_gt_boxes_list' in info:
            res["lidar"]["sweep_points_list"] = sweep_points_list
            res["lidar"]["sweep_annotations"] = {
                "names_list": [],
                "tokens_list": [],
                "boxes_list": [],
            }
            for t in range(len(info["sweep_gt_boxes_list"])):
                mask = info["sweep_num_lidar_pts_list"][t] > 0
                names = info["sweep_gt_names_list"][t]
                tokens = info["sweep_gt_tokens_list"][t]
                res["lidar"]["sweep_annotations"]["names_list"].append(names[mask])
                res["lidar"]["sweep_annotations"]["tokens_list"].append(tokens[mask])

                gt_boxes = info["sweep_gt_boxes_list"][t][mask]
                if self._with_velocity:
                    gt_velocity = info["sweep_gt_velocity_list"][t][mask]
                    nan_mask = np.isnan(gt_velocity[:, 0])
                    gt_velocity[nan_mask] = [0.0, 0.0]
                    gt_boxes = np.concatenate([gt_boxes, gt_velocity], axis=-1)
                res["lidar"]["sweep_annotations"]["boxes_list"].append(gt_boxes)
    ...
    def evaluation(self, detections, output_dir, clean_after=True):
        """kitti evaluation is very slow, remove it.
        """
        # res_kitti = self.evaluation_kitti(detections, output_dir)
        res_nusc = self.evaluation_nusc(detections, output_dir, clean_after)
        res = {
            "results": {
        ...
        if not test:
            ...
            for ann in annotations:
                ...
                if len(attr_tokens) == 0:
                    ...
                    # so far, all examples have only one attribute
                    # this might break in the future when one object has multiple attributes
                    attr_dict = nusc.get('attribute', attr_tokens[0])
                    attrs.append(attr_dict['name'])
            attrs = np.array(attrs)
            info["gt_attrs"] = attrs

            #########################################################################
            # # Here we work on extracting trajectories of objects
            # # Each object will have a unique ID
            if with_sweep_info:
                sweep_gt_boxes_list = [info["gt_boxes"]]
                sweep_gt_names_list = [info["gt_names"]]
                sweep_gt_velocity_list = [info["gt_velocity"]]
                tokens = np.array([b.token for b in boxes])
                sweep_gt_tokens_list = [tokens]
                sweep_num_lidar_pts_list = [info["num_lidar_pts"]]
                sweep_num_radar_pts_list = [info["num_radar_pts"]]

                for t in range(len(sweeps)):
                    _, boxes, _ = nusc.get_sample_data(sweeps[t]['sample_data_token'])
                    for i in range(len(boxes)):
                        boxes[i].rotate(Quaternion(matrix=sweeps[t]['sweep2lidar_rotation']))
                        boxes[i].translate(sweeps[t]['sweep2lidar_translation'])

                    annotations = [
                        nusc.get('sample_annotation', b.token) for b in boxes
                    ]
                    ...
                    sweep_gt_boxes_list.append(gt_boxes)
                    sweep_gt_names_list.append(names)
                    sweep_gt_velocity_list.append(velocity.reshape(-1,2))
                    sweep_gt_tokens_list.append(tokens)
                    sweep_num_lidar_pts_list.append(np.array([a["num_lidar_pts"] for a in annotations]))
                    sweep_num_radar_pts_list.append(np.array([a["num_radar_pts"] for a in annotations]))

                info["sweep_gt_boxes_list"] = sweep_gt_boxes_list
                info["sweep_gt_names_list"] = sweep_gt_names_list
                info["sweep_gt_velocity_list"] = sweep_gt_velocity_list
                info["sweep_gt_tokens_list"] = sweep_gt_tokens_list
                info["sweep_num_lidar_pts_list"] = sweep_num_lidar_pts_list
                info["sweep_num_radar_pts_list"] = sweep_num_radar_pts_list
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", max_sweeps=10, with_sweep_info=False):
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, test, max_sweeps=max_sweeps, with_sweep_info=with_sweep_info)
    metadata = {
        "version": version,
    }
    if test:
        test_info_name = "infos_test_with_sweep_info.pkl" if with_sweep_info else "infos_test.pkl"
        print(f"test sample: {len(train_nusc_infos)}")
        data = {
            "infos": train_nusc_infos,
            "metadata": metadata,
        }
        with open(root_path / test_info_name, 'wb') as f:
            pickle.dump(data, f)
    else:
        train_info_name = "infos_train_with_sweep_info.pkl" if with_sweep_info else "infos_train.pkl"
        val_info_name = "infos_val_with_sweep_info.pkl" if with_sweep_info else "infos_val.pkl"
        print(
        ...
        with open(root_path / train_info_name, 'wb') as f:
            pickle.dump(data, f)
        data["infos"] = val_nusc_infos
        with open(root_path / val_info_name, 'wb') as f:
            pickle.dump(data, f)
