...
parser.add_argument('--metric', type=str, default='trans_err')
...
for name, method in zip(names, methods):
    ...
    for cls in classes:
        ...
        err = cache[method]["label_tp_errors"][cls][args.metric]
        errs.append(err)
        print('{:.3f}'.format(err), end=delim)
    # mAP = sum(APs)/len(APs)
    mErr = sum(errs) / len(errs)
    print('{:.3f}'.format(mErr))
