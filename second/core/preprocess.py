...
def random_flip(gt_boxes, points, origins, probability=0.5, random_flip_x=True, random_flip_y=True):
    ...
    return gt_boxes, points, origins


def global_scaling_v2(gt_boxes, points, origins, min_scale=0.95, max_scale=1.05):
    noise_scale = np.random.uniform(min_scale, max_scale)
    points[:, :3] *= noise_scale
    origins[:, :3] *= noise_scale
...
def global_rotation_v2(gt_boxes, points, origins, min_rad=-np.pi / 4,
                       max_rad=np.pi / 4):
    ...
    origins[:, :3] = box_np_ops.rotation_points_single_angle(
        origins[:, :3], noise_rotation, axis=2)
...
def global_translate_(gt_boxes, points, origins, noise_translate_std):
    """
