...
def example_convert_to_torch(example, dtype=torch.float32,
    ...
    for k, v in example.items():
        ...
        elif k in ["visibility"]: 
            example_torch[k] = torch.tensor(
                v, dtype=torch.int8, device=device)
        elif k in ["logodds"]: 
            example_torch[k] = torch.tensor(
...
def _worker_init_fn(worker_id):
    ...
    # print(f"!!!!!!!!!!!!!!!!!!!!! THIS NEEDS TO BE REVISITED !!!!!!!!!!!!!!!!!!!!!")
    ...
    try:
        while True:
            ...
            for example in dataloader:
                ...
                # print(f'example["anchors"].shape: {example["anchors"].shape}')
...
def evaluate(config_path,
    ...
    with open(result_path_step / "result.pkl", 'wb') as f:
        pickle.dump(detections, f)
    result_dict = eval_dataset.dataset.evaluation(detections, str(result_path_step), clean_after)
