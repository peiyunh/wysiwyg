...
class VoxelNet(nn.Module):
    def __init__(self,
                 ...
                 rpn_use_groupnorm=False,
                 rpn_num_groups=32,
                 vpn_class_name="VPN",
                 vpn_num_input_features=-1,
                 vpn_layer_nums=[3, 5, 5],
                 vpn_layer_strides=[2, 2, 2],
                 vpn_num_filters=[128, 128, 256],
                 vpn_upsample_strides=[1, 2, 4],
                 vpn_num_upsample_filters=[256, 256, 256],
                 vpn_use_groupnorm=False,
                 vpn_num_groups=32,
                 use_norm=True,
                 # use_groupnorm=False,
                 # num_groups=32,
        ...
        self.rpn = rpn.get_rpn_class(rpn_class_name)(
            ...
            use_groupnorm=rpn_use_groupnorm,
            num_groups=rpn_num_groups,
            box_code_size=target_assigner.box_coder.code_size,
            num_direction_bins=self._num_direction_bins)
        if vpn_num_input_features > 0:
            self.vpn = rpn.get_rpn_class(rpn_class_name)(
                use_norm=True,
                num_class=num_class,
                layer_nums=vpn_layer_nums,
                layer_strides=vpn_layer_strides,
                num_filters=vpn_num_filters,
                upsample_strides=vpn_upsample_strides,
                num_upsample_filters=vpn_num_upsample_filters,
                num_input_features=vpn_num_input_features,
                num_anchor_per_loc=target_assigner.num_anchors_per_location,
                encode_background_as_zeros=encode_background_as_zeros,
                use_direction_classifier=use_direction_classifier,
                use_groupnorm=vpn_use_groupnorm,
                num_groups=vpn_num_groups,
                box_code_size=target_assigner.box_coder.code_size,
                num_direction_bins=self._num_direction_bins)
    ...
    def network_forward(self, voxels, num_points, coors, batch_size, logodds=None):
    ...
    def forward(self, example):
        ...
        # coors: [num_voxels, 4]
        preds_dict = self.network_forward(voxels, num_points, coors, batch_size_dev, logodds)
        # need to check size.
        ...
        # print(f"batch_anchors.shape: {batch_anchors.shape}, box_preds.shape: {box_preds.shape}")
